    ...
    lineas.forEach((linea) => {
      ...
      if (categoriaMatch) {
        ...
        if (cat.includes('DILDOS')) {
          ...
          cat = 'ANILLOS FUNDAS';
        } else if (cat.includes('BOMBAS') || cat.includes('MASTURBAD')) {
          cat = 'MASTURBADORES BOMBAS';
        } else if (cat.includes('RETARDANTE')) {
          cat = 'RETARDANTES';
        } else if (cat.includes('VIGORIZANTE')) {
          cat = 'VIGORIZANTES SEXUALES';
        } else if (cat.includes('ACCESORIO') || cat.includes('ESTIMULANTE')) {
          cat = 'ESTIMULANTES';
        } else if (cat.includes('FETICHE')) {
          cat = 'ARNES FETISH';
        }
        producto.categoria = cat;
      }

      const descripcionMatch = linea.match(/DESCRIPCION:\s*((?:(?!PRECIO:)[^\n]+)/?)(?:\n|$)/);
      if (descripcionMatch) {
      ...
      if (precioBaseMatch) {
        const precio = parseFloat(precioBaseMatch[1].replace(',', '.'));
      ...
      if (precioPromoMatch) {
        const precio = parseFloat(precioPromoMatch[1].replace(',', '.'));
        if (!isNaN(precio) && precio > 0) {
